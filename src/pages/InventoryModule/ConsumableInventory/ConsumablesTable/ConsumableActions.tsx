...
import { useDeleteConsumableMutation } from "../../../../features/api/consumableInventorySlice";
import { ErrorResponse } from "react-router-dom";
import ConsumableUpdate from "../ConsumableUpdate";
import {
  FaTag,
  FaDollarSign,
  FaCube,
...
interface ConsumableActionProp {
  data: {
    uid: string;
    name: string;
    descriptions: string;
...
const ConsumableActions: React.FC<ConsumableActionProp> = ({ data }) => {
  ...
  const [deleteModalOpened, { open: openDelete, close: closeDelete }] =
    useDisclosure(false);
  const [deleteConsumable, { isLoading: deleteLoading, error }] =
    useDeleteConsumableMutation();
  const { formatDate } = useFormatDate();

  const handleDelete = async () => {
    try {
      const response = await deleteConsumable({ uid: data.uid }).unwrap();
  ...
  return (
    <>
      ...
      <Modal
        ...
        size="lg"
      >
        <ConsumableUpdate uid={data.uid} closeEdit={closeEdit} />
      </Modal>
